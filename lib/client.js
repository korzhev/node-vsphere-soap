...
Client.prototype.runCommand = function (command, arguments) {
    ...
    if( self.client && (self.status == 'ready' || self.status == 'connecting')) {
    ...
    soap.createClient(self._vcUrl, self.clientopts, function (err, client) {
        ...
        self.runCommand('RetrieveServiceContent', { _this: 'ServiceInstance' })
            .once('result', function (result, raw, soapHeader) {
                ...
                var loginArgs = Object.assign({ _this: self.sessionManager }, self._loginArgs);
...
function _soapErrorHandler( self, emitter, command, arguments, err ) {
    var err_data = 'general error';
    if(err) {
        try {
            err_data = JSON.parse(xmlParser.toJson(err.body))['soapenv:Envelope']['soapenv:Body']['soapenv:Fault'].faultstring
        } catch (e) {
            err_data = err.body || err;
        }
    }
    if(typeof err_data == 'string' && err_data.match( /session is not authenticated/ ) ) {
        self.status = 'disconnected';
        process.removeAllListeners('beforeExit');
        if(self.reconnectCount < 10) { // TODO WTF?
